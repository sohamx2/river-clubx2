...
export type Action = {type:'start'} | {type:'act';move:'fold'|'check'|'call'|'raise';amount?:number;expectedAction:number} | {type:'propose';kind:'bomb'|'bounty'|'sevenDeuce'|'ocean';amount:number} | {type:'vote';voteId:string;yes:boolean} | {type:'theme';theme:Theme} | {type:'seats';maxPlayers:number} | {type:'sit';out:boolean} | {type:'rebuy'} | {type:'leave'} | {type:'cancelBounty'};
